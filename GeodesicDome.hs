...
interpolate (x0, y0, z0) (x1, y1, z1) t =
...
subdivide n (a, b, c) =
  concatMap makeTriangles . map fromIntegral $ [1 .. n - 1]
  where
    makeTriangles i =
      let f01 = interpolate a b (i / fromIntegral n)
          f02 = interpolate a c (i / fromIntegral n)
          f11 = interpolate a b ((i + 1) / fromIntegral n)
          f12 = interpolate a c ((i + 1) / fromIntegral n)
          makeTopTriangle j =
            ( interpolate f01 f02 (j / i)
            , interpolate f01 f02 ((j + 1) / i)
            , interpolate f11 f12 ((j + 1) / (i + 1)))
          makeBottomTriangle j =
            ( interpolate f11 f12 (j / (i + 1))
            , interpolate f11 f12 ((j + 1) / (i + 1))
            , interpolate f01 f02 (j / i))
       in map makeTopTriangle [0 .. i - 1] ++
          map makeBottomTriangle [0 .. i] ++
          [ ( a
            , interpolate a b (1 / fromIntegral n)
            , interpolate a c (1 / fromIntegral n))
          ]
...
main = do
  ...
  if length args /= 1
    then do
      print
        "Please provide the number of subdivision, for example: ./GeodesicDome 5"
      exitWith (ExitFailure 1)
    else print . dome . read . (!! 0) $ args
